...
// ── Input normaliser — strips full URLs to bare username ───────────────────────
// Handles:
//   https://github.com/username      → username
//   https://github.com/username/     → username
//   https://leetcode.com/u/username/ → username
//   https://leetcode.com/username    → username
//   plain username                   → username (unchanged)
function normaliseUsername(raw: string, platform: "github" | "leetcode"): string {
  ...
  try {
    ...
    if (platform === "leetcode") {
      ...
      return (parts[0] === "u" ? parts[1] : parts[0]) ?? trimmed;
    }
  } catch {
...
// ── GitHub via backend proxy ──────────────────────────────────────────────────
// The browser calling GitHub directly hits 60 req/hr rate limits + CORS issues.
// Our FastAPI backend proxies it server-side with proper headers.
async function fetchGitHub(rawInput: string): Promise<GitHubData> {
  ...
  if (!res.ok) {
    ...
    throw new Error(err.detail ?? `GitHub error ${res.status}`);
  }
...
// The alfa-leetcode-api sometimes has CORS and cold-start issues from the browser.
// Backend proxy is more reliable and avoids those constraints.
async function fetchLeetCode(rawInput: string): Promise<LeetCodeData> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ detail: `HTTP ${res.status}` }));
    throw new Error(err.detail ?? `LeetCode error ${res.status}`);
  }
  const data = await res.json();
  return {
    username:      data.username,
...
export default function ProfilePage() {
  ...
  const loadGitHub = async () => {
    ...
    try {
      // fetchGitHub internally normalises URLs → bare username
      ...
      toast(`GitHub: ${e instanceof Error ? e.message : "Unknown error"}`, "error");
    }
  };

  // ── LeetCode (real API, falls back to error) ────────────────────────────────
  const loadLeetCode = async () => {
    ...
    try {
      // fetchLeetCode internally normalises URLs → bare username
      ...
      toast(`LeetCode: ${e instanceof Error ? e.message : "Unknown error"}`, "error");
    }
  };
  ...
  return (
    <>
      ...
      <PageWrapper>
        {/* Header */}
        <div className="mb-8">
          <motion.h1 initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="text-3xl font-bold mb-1">
            Profile{" "}
            <span className="bg-gradient-to-r from-violet-400 to-cyan-400 bg-clip-text text-transparent">Builder</span>
          </motion.h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
          ...
          <SourcePanel icon={<FileText className="w-5 h-5" />} title="Resume" color="bg-violet-500/15 text-violet-400" border="border-violet-500/15" status={resumeState}>
            <input ref={fileInputRef} type="file" accept=".pdf" className="hidden"
              onChange={e => e.target.files?.[0] && processResume(e.target.files[0])} />
            ...
            <AnimatePresence mode="wait">
              {resumeState === "idle" && (
                <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  className="flex flex-col items-center justify-center gap-3 py-8 rounded-2xl border-2 border-dashed
                    border-violet-500/20 bg-violet-500/5 cursor-pointer hover:border-violet-500/40
                    hover:bg-violet-500/8 transition-all text-center"
                >
              ...
              {resumeState === "loading" && (
                <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><Skeleton /></motion.div>
              )}
              {(resumeState === "done" || resumeState === "error") && resumeData && (
                <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => { setResumeState("idle"); setResumeData(null); setUploadedFile(null); }}
                      className="text-white/30 hover:text-red-400 transition-colors"><X className="w-4 h-4" /></button>
                  </div>
          ...
          <SourcePanel icon={<GitFork className="w-5 h-5" />} title="GitHub" color="bg-cyan-500/15 text-cyan-400" border="border-cyan-500/15" status={githubState}>
            <div className="flex gap-2">
              <input
                ...
                onChange={e => { setGithubUsername(e.target.value); if (githubState !== "idle") setGithubState("idle"); }}
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm
                  text-white/80 placeholder:text-white/20 outline-none focus:border-cyan-500/40 transition-colors"
              />
              <motion.button
                onClick={loadGitHub} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            ...
            <AnimatePresence mode="wait">
              {githubState === "idle" && (
                <motion.p key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="text-xs text-white/25 text-center py-4">Enter a username to fetch public repos</motion.p>
              )}
              {githubState === "loading" && (
                <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><Skeleton /></motion.div>
              )}
              {githubState === "done" && githubData && (
                <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { l: "Repos",    v: githubData.repoCount,          c: "text-cyan-400"   },
                      { l: "Stars",    v: githubData.totalStars,          c: "text-amber-400"  },
                      { l: "Followers",v: githubData.followers,           c: "text-violet-400" },
                      { l: "Languages",v: githubData.topLanguages.length, c: "text-emerald-400"},
                    ].map(({ l, v, c }) => (
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {githubData.topLanguages.slice(0, 5).map(l => (
                      <span key={l.name} className="text-[10px] px-2 py-0.5 rounded-full bg-cyan-500/12 border border-cyan-500/20 text-cyan-300">{l.name}</span>
                    ))}
                  </div>
                </motion.div>
              )}
              {githubState === "error" && (
                <motion.div key="err" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  className="text-sm text-red-400/80 flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4 shrink-0" />User not found or API limit hit.
                </motion.div>
              )}
            </AnimatePresence>
          </SourcePanel>
          ...
          <SourcePanel icon={<Code2 className="w-5 h-5" />} title="LeetCode" color="bg-amber-500/15 text-amber-400" border="border-amber-500/15" status={leetcodeState}>
            <div className="flex gap-2">
              <input
                ...
                onChange={e => { setLeetcodeUsername(e.target.value); if (leetcodeState !== "idle") setLeetcodeState("idle"); }}
                onKeyDown={e => e.key === "Enter" && loadLeetCode()}
                placeholder="username or leetcode.com/u/user"
                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm
                  text-white/80 placeholder:text-white/20 outline-none focus:border-amber-500/40 transition-colors"
              />
              <motion.button
                onClick={loadLeetCode} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                disabled={leetcodeState === "loading"}
                className="px-3 py-2 rounded-xl bg-amber-500/15 border border-amber-500/25 text-amber-400
            ...
            <AnimatePresence mode="wait">
              {leetcodeState === "idle" && (
                <motion.p key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="text-xs text-white/25 text-center py-4">Enter username to load profile</motion.p>
              )}
              ...
              {leetcodeState === "done" && leetcodeData && (
                <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {[
                      { l: "Easy",   v: leetcodeData.easy,   c: "#10b981" },
                      { l: "Medium", v: leetcodeData.medium, c: "#f59e0b" },
                      { l: "Hard",   v: leetcodeData.hard,   c: "#ef4444" },
                    ].map(({ l, v, c }) => (
                      <div key={l} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 h-1.5 rounded-full bg-white/8 overflow-hidden">
                          <motion.div className="h-full rounded-full"
                            style={{ background: c }}
                            initial={{ width: 0 }}
                            animate={{ width: `${(v / leetcodeData.totalSolved) * 100}%` }}
                  ...
                  <p className="text-xs text-amber-300/70 pt-1">
                    ⚡ Contest rating: <span className="font-bold">{leetcodeData.contestRating}</span>
                    <span className="text-white/25 ml-1">(estimated)</span>
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          {anyLoaded && (
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-violet-400/70 px-3">
                  Unified Profile
                </span>
                <div className="flex-1 h-px bg-gradient-to-r from-transparent via-violet-500/30 to-transparent" />
              </div>
        ...
        {!anyLoaded && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
            ...
            <p className="text-white/30 text-sm max-w-sm">
              Load at least one source above to generate your unified candidate profile.
            </p>
          </motion.div>
        )}
      </PageWrapper>
